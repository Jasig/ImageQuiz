 ...
 * @author  Moz123
 */
public class ProgressClass extends javax.swing.JDialog {
    ...
    public ProgressClass() {
        ...
        addEscapeListener(this);//Added by Anurag
    }
    ...
    public void ShowResults() { 
       ...
       // this.jTextArea1.setText(results);
        taResultWindow.setContentType("text/html");
    ...
    public void StoreResult(QuizResultClass mResult, String grade,boolean fontItalic){
        ...
        header += "<br>" + mResult.date_time + "<br>";
        header += "Number of images: " + mResult.numImages + "<br>";
        header += "Spelling Sensitivity: " + ((10 - mResult.sensitivity) * 10) + "%" + "<br>";
        header += "Image Display Time: " + String.format( "%.1f", mResult.displayTime) + "s<br>";
        ...
        for(Iterator it = mFamilyKeys.keySet().iterator(); it.hasNext();){
            ...
            if(sub_levels.size() > 0){
                for(int i = 0; i < sub_levels.size(); i++){
                     if(fontItalic){
                    sub_string += "<PRE>"+tab2+"<i>" + sub_levels.get(i).toString() + ':'+"</i>";//Added by preethy
                    }
                    else{
                    sub_string += "<PRE>"+tab2+ sub_levels.get(i).toString() + ':';
                     }
                    double score = Double.parseDouble(scores.get(i).toString());
                    total += score;
                    denom++;
                    sub_string += "  " + to_percent(Double.toString(score)) + "</PRE>"; 
                }
                result_string += "<PRE>"+tab+ fam + ":  " + to_percent(Double.toString(total/denom)) + "</PRE>" + sub_string;
            } 
            else {  //Family only, no sub-levels
                total = Double.parseDouble(scores.get(0).toString());
                denom = 1;
                result_string += "<PRE>"+tab+ fam + ":  " + to_percent(Double.toString(total/denom)) + "<br></PRE>";
            }
        }
        result_string += "" + "Overall Grade: " + grade + "<br>";
        myResults.add(0,  header + result_string + "<br>");
    }
