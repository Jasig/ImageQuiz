 ...
 */
public class ImageNamingTestScript {
    SessionInfo mSessionInfo;
    int mMode;
    boolean Running = true;
    boolean fontItalic=false;
    public ImageNamingTestScript(SessionInfo session, int mode) {
    ...
    public void RunScript(){
        ...
        for(int i = 0; i<mSessionInfo.mFileNames.length; i++){
            ...
            while(Running){
                ...
                if(SpellingClass.getPassOrFail(SessionInfo.SpellingValue, tempPart, user_response)){
                    ...
                    mGrade.RightAnswer();
                    //RandomResponse.ShowPositiveResponse(mSessionInfo.mParentForm);//Added by preethy on 09-03-2012
                    break;// exit while - do next image(if exists)
                } else{
        ...
        // GRADING
        ScriptResult rs = new ScriptResult("Image Naming", mSessionInfo.mTaxaLevel, mSessionInfo.mTaxa, mSessionInfo.mUserName, mGrade.FinalGrade(),
                mSessionInfo.mScriptName, mSessionInfo.mSessionName, mGrade.mTotal, mSessionInfo.mImageCollection.NumberOfImages(), mSessionInfo.mScriptFileName);
        rs.writeResults();
